...
function upstreamTopLevelAwaitTests({ TEST_DIR, tsNodeUnderTest, }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const service = tsNodeUnderTest.create(Object.assign(Object.assign({}, replService.evalAwarePartialHost), { project: `${TEST_DIR}/tsconfig.json`, experimentalReplAwait: true, transpileOnly: true, compilerOptions: {
                target: semver.gte(helpers_1.ts.version, '3.0.1')
                    ? 'es2018'
        ...
        for (const [input, expected = [`${input}\r`], options = {},] of testCases) {
            ...
            const lines = yield runAndWait(toBeRun);
            if (Array.isArray(expected)) {
                if (expected.length === 1)
                    expected.push('undefined');
                if (lines[0] === input)
                    lines.shift();
                (0, chai_1.expect)(lines).to.eqls([...expected, PROMPT]);
            }
            else if ('line' in options) {
                (0, chai_1.expect)(lines[toBeRun.length + options.line]).to.eqls(expected);
            }
            else {
                const echoed = toBeRun.map((a, i) => `${i > 0 ? '... ' : ''}${a}\r`);
                (0, chai_1.expect)(lines).to.eqls([...echoed, expected, PROMPT]);
            }
        }
    });
}
